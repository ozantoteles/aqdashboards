...
from example_plots import plot_scatter1_temp, plot_scatter1_hum, plot_scatter1_pm1, plot_scatter1_pm2_5, plot_scatter1_pm10, plot_scatter1_voc, plot_scatter1_nox, plot_scatter1_co2, plot_scatter1_bat, plot_scatter2_bat, plot_scatter2_co2, plot_scatter2_hcho, plot_scatter2_hum, plot_scatter2_nox, plot_scatter2_pm1, plot_scatter2_pm10, plot_scatter2_pm2_5, plot_scatter2_temp, plot_scatter2_voc, plot_scatter3_co2, plot_scatter3_hum, plot_scatter3_temp
...
dynamodb = boto3.resource('dynamodb', aws_access_key_id=login["aws_access_key_id"],                           aws_secret_access_key=login["aws_secret_access_key"], region_name=login["region_name"])

table = dynamodb.Table('CAirAirQualitySensorHistoryTable')
table_co2 = dynamodb.Table('ApplianceHourlyGraphDataTable')

applianceID_aio = "F999914256286571835718"
applianceID_co2 = "E999984469256823117735"
applianceID_cair = "F999909799992438052710"
...
def prepareSingleDevData_aio(applianceID):
    import time
    nowMillis = int(round(time.time() * 1000))
    now = int(round(time.time()))
    h1Millis = 3600000
    h1 = 3600
    startTimeMillis = nowMillis - h1Millis * 6
    startTime = now - h1 * 6
    endTime = now
...
def prepareSingleDevData_co2(applianceID):
    import time
    nowMillis = int(round(time.time() * 1000))
    now = int(round(time.time()))
    h1Millis = 3600000
    ...
    #response = table.query(TableName='CAirAirQualitySensorHistoryTable',                            KeyConditionExpression=Key('applianceId').eq(applianceID) & Key('timestamp').gte(startTime))
    
    response = table.query(TableName='ApplianceHourlyGraphDataTable',
                           KeyConditionExpression=Key('applianceId').eq(applianceID_co2) & Key('hour').lte(hour_now))
    
    data = response['Items']

    print("co2 devID: ", applianceID)
    print("startTime: ", startTime)
    print("endTime: ", endTime)
    #print(data[-1])
    print("len(data): ", len(data))
    ...
    for i in range(len(data)):
        try:
            ts = float(data[i]['created_timestamp'])  # ['N']
            time = pd.to_datetime(ts, unit='s') + timedelta(hours=3)
            #payload = json.loads(data[i]['payload'])
            CAIRCO2LEVEL = data[i]['average_co2_level']
            CAIRRHTLEVEL_EXTERNAL_TEMP = data[i]['average_temp']
            CAIRRHTLEVEL_EXTERNAL_HUM = data[i]['average_humidity']
            parsed.append(
                [CAIRCO2LEVEL,
                CAIRRHTLEVEL_EXTERNAL_TEMP, 
                CAIRRHTLEVEL_EXTERNAL_HUM])
...
sidebar = dac.Sidebar(
    dac.SidebarMenu(
        [
            ...
            dac.SidebarMenuItem(id='tab_basic_boxes1', label='Air Master Allergen', icon='box'),
            dac.SidebarMenuItem(id='tab_basic_boxes2', label='Air Master Industrial', icon='box'),
            dac.SidebarMenuItem(id='tab_basic_boxes3', label="CO2 Device", icon='box'),
...
def update_box_graph(value):
    errTime, time, data = prepareSingleDevData_cair(applianceID_cair)
...
@app.callback(
    ...
    [Input('controlbar-slider', 'value')])
def update_box_graph(value):
    errTime, time, data = prepareSingleDevData_aio(applianceID_aio)
    return plot_scatter2_hcho(time, data), plot_scatter2_co2(time, data), plot_scatter2_temp(time, data), plot_scatter2_hum(time, data), plot_scatter2_pm1(time, data), plot_scatter2_pm2_5(time, data), plot_scatter2_pm10(time, data), plot_scatter2_voc(time, data), plot_scatter2_nox(time, data), plot_scatter2_bat(time, data)
...
@app.callback(
    ...
    [Input('controlbar-slider', 'value')])
def update_box_graph(value):
    errTime, time, data = prepareSingleDevData_co2(applianceID_co2)
    return plot_scatter3_co2(time, data), plot_scatter3_temp(time, data), plot_scatter3_hum(time, data)
...
if __name__ == '__main__':
    app.run_server(debug=True, host='0.0.0.0', port=9093)
